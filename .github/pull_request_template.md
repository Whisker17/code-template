...
- [ ] `dev` (finished version-integration `release/v*` merge-back) → **merge commit**, never squash — **human gate**
...
- [ ] After this merges, `dev` fans out into every live `release/v*` integration
      ...
      (`docs/GIT_WORKFLOW.md` § Fan-out) — a governance rule is only in force on branches that carry it.
